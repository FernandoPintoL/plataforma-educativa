// Página Edit de Lección - Formulario Personalizado con Soporte Multimedia
...
import type { LeccionEditPageProps, LeccionFormData } from '@/domain/modulos'
...
export default function Edit({ leccion, modulos, recursos, recursos_asociados }: LeccionEditPageProps) {
    const { data, setData, put, processing, errors } = useForm<LeccionFormData>({
        modulo_educativo_id: leccion.modulo_educativo_id,
        titulo: leccion.titulo,
        contenido: leccion.contenido || '',
        tipo: leccion.tipo,
        orden: leccion.orden,
        duracion_estimada: leccion.duracion_estimada || 0,
        video_url: leccion.video_url || '',
        video_proveedor: leccion.video_proveedor,
        es_obligatoria: leccion.es_obligatoria,
        permite_descarga: leccion.permite_descarga,
        estado: leccion.estado,
        recursos: recursos_asociados || []
    })
    ...
    const handleSubmit = (e: React.FormEvent, publicar = false) => {
        ...
        const newData = { ...data }
        if (publicar) {
            newData.estado = 'publicado'
        }
        put(`/lecciones/${leccion.id}`, {
            data: newData,
            preserveScroll: true
        })
    }
    ...
    return (
        <AppLayout
            breadcrumbs={[
                ...
                { title: 'Editar', href: `/lecciones/${leccion.id}/edit` }
            ]}
        >
            <Head title={`Editar: ${leccion.titulo}`} />
            ...
            <div className="py-12">
                <div className="mx-auto max-w-4xl sm:px-6 lg:px-8 space-y-6">
                    ...
                    <div className="flex items-center justify-between">
                        ...
                        <Badge variant={
                            leccion.estado === 'publicado' ? 'default' :
                            leccion.estado === 'archivado' ? 'outline' : 'secondary'
                        }>
                            {leccion.estado === 'publicado' ? 'Publicado' :
                             leccion.estado === 'archivado' ? 'Archivado' : 'Borrador'}
                        </Badge>
                    </div>
                    ...
                    <form onSubmit={(e) => handleSubmit(e, false)}>
                        ...
                        <Card className="mb-6">
                            ...
                            <CardContent className="space-y-4">
                                ...
                                {/* Estado */}
                                <div>
                                    <Label htmlFor="estado">Estado de la Lección</Label>
                                    <Select
                                        value={data.estado}
