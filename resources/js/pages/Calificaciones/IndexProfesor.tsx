...
export default function IndexProfesor({ calificaciones, cursos, filters = {} }: Props) {
  ...
  const handleDelete = (calificacionId: number) => {
    if (confirm('¿Estás seguro de que deseas eliminar esta calificación?')) {
      router.delete(`/calificaciones/${calificacionId}`, {
        onSuccess: () => {
          router.get('/calificaciones', {}, { preserveScroll: true });
        },
      });
    }
  };
  ...
  const calcularEstadisticas = () => {
    const data = calificaciones.data;
    if (data.length === 0) return { promedio: 0, maxima: 0, minima: 0 };

    const puntajes = data.map(cal => cal.puntaje);
    const promedio = (puntajes.reduce((a, b) => a + b, 0) / puntajes.length).toFixed(1);
    const maxima = Math.max(...puntajes);
    const minima = Math.min(...puntajes);
      ...
      <div className="container mx-auto py-8 px-4">
        ...
        <div className="mb-8">
          <div className="flex items-center gap-3 mb-6">
            ...
            <div>
              <h1 className="text-3xl font-bold">Calificaciones Asignadas</h1>
              <p className="text-muted-foreground">
                Gestiona las calificaciones que has otorgado a tus estudiantes
              </p>
            </div>
          </div>

          {/* Estadísticas de Desempeño de la Clase */}
          {calificaciones.data.length > 0 && (
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
              <div className="bg-card rounded-lg border p-4">
                <div className="text-sm text-muted-foreground mb-1">Promedio de Clase</div>
                <div className="text-3xl font-bold text-primary">{stats.promedio}</div>
              </div>
              <div className="bg-card rounded-lg border p-4">
                <div className="text-sm text-muted-foreground mb-1">Calificación Máxima</div>
                <div className="text-3xl font-bold text-green-600">{stats.maxima}</div>
              </div>
              <div className="bg-card rounded-lg border p-4">
                <div className="text-sm text-muted-foreground mb-1">Calificación Mínima</div>
                <div className="text-3xl font-bold text-red-600">{stats.minima}</div>
              </div>
              <div className="bg-card rounded-lg border p-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="md:col-span-2">
              <div className="relative">
                ...
                <Input
                  placeholder="Buscar por estudiante o título de tarea..."
                  value={search}
        ...
        {/* Tabla de Calificaciones del Profesor */}
                    ...
                    <TableRow key={calificacion.id}>
                      <TableCell>
                        <div className="text-sm">
                          <div className="font-medium">
                            {calificacion.trabajo.estudiante.name} {calificacion.trabajo.estudiante.apellido}
                          </div>
                        </div>
                      </TableCell>
                      <TableCell>
                      ...
                      <TableCell>
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-sm">{formatFecha(calificacion.fecha_calificacion)}</span>
                        </div>
                      </TableCell>
